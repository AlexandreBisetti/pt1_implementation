...
Livre.livreInteretDispo = (personneID, result) => {
  sql.query(`SELECT DISTINCT Livre.* FROM Livre, Auteur, Auteur_livre, Interet_Livre, Client WHERE Livre.ID_livre = Auteur_livre.ID_livre and Auteur_livre.ID_auteur = Auteur.ID_auteur and Interet_Livre.ISBN_livre = Livre.ISBN and estInteresse_livre = 1 and Client.ID_personne = ${personneID} and Livre.Verifie = 1 and Livre.ID_Personne_Echange IS NULL`, (err, res) => {
    if (err) {
      console.log("erreur: ", err);
      result(null, err);
      return;
